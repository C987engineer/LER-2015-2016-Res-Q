...
//Handles basic functions of the robot movement
    ...
    public void setRunMode(String motorName, DcMotorController.RunMode runMode) {
        motors.get(motorName).setMode(runMode);
    }

    public void setRunMode(String[] motorNames, DcMotorController.RunMode runMode) {
        for (int i = 0; i < motorNames.length; i++) {
    ...
    public void setRunMode(String[] motorNames, DcMotorController.RunMode[] runModes) {
        for (int i = 0; i < motorNames.length; i++) {
            motors.get(motorNames[i]).setMode(runModes[i]);
        }
    }

    public void setAllRunModes(DcMotorController.RunMode runMode) {
        Set keys = motors.keySet();
        for (Iterator<String> i = keys.iterator(); i.hasNext(); ) {
    ...
    public int moveDistanceEncoder(float distance, String[] drivetrain) {
        setRunMode(drivetrain, DcMotorController.RunMode.RESET_ENCODERS);
        ...
        setRunMode(drivetrain, DcMotorController.RunMode.RUN_TO_POSITION);
        //DbgLog.msg(MessageFormat.format("moveDistanceEncoder: Set {0} to run mode {1}", Arrays.toString(drivetrain), getMotor(drivetrain[0]).getMode()));
